...
void DistanceToFlagPlugin::Load(physics::WorldPtr world, sdf::ElementPtr sdf) {
  int nbFlags = sdf->GetElement("nbFlags")
                    ->Get<int>(); // get the number of flags in the environment
  ...
  _dtf_service = _nh->advertiseService(
      "distanceToFlag", &DistanceToFlagPlugin::distanceToFlag,
      this); // create the service "distanceToFlag"
  _nh->setParam("nb_flags", nbFlags);
}

bool DistanceToFlagPlugin::distanceToFlag(
    evry_project_plugins::DistanceToFlag::Request &req,
    evry_project_plugins::DistanceToFlag::Response &res) {
  float dx = _flags_poses[req.id_flag - 1].x -
             req.agent_pose.x; // difference of coordinates between the robot
                               // and the specified flag along the x axis
  float dy = _flags_poses[req.id_flag - 1].y -
             req.agent_pose.y; // difference of coordinates between the robot
                               // and the specified flag along the y axis
